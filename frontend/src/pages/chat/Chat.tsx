...
const Chat = ({ embedDisplay }: { embedDisplay: boolean }) => {
  ...
  const SPEECH_ENABLED =
    appStateContext?.state.frontendSettings?.speech_enabled;
    ...
    <div className={styles.container} role="main">
      {showAuthMessage ? (
        ...
        <div className={styles.container}>
          <div
            className={
              embedDisplay ? styles.chatContainerEmbed : styles.chatContainer
            }
          >
            {!messages || messages.length < 1 ? (
              <div className={styles.chatEmptyState}>
                {appStateContext?.state.frontendSettings
                  ?.frontpage_show_image && (
                  <Image
                    src={
                      appStateContext?.state.frontendSettings
                        ?.frontpage_image_url
                ...
                <span className={styles.title}>
                  {appStateContext?.state.frontendSettings?.frontpage_heading}
                </span>
                {appStateContext?.state.frontendSettings
                ...
                {appStateContext?.state.frontendSettings?.frontpage_links?.map(
                  (link, index) => (
                ...
                <SuggestionButtons onButtonClick={sendChatQuestion} />
              </div>
            ) : (
                  ...
                  <div key={`answer-${index}`}>
                    {answer.role === "user" ? (
                      <div className={styles.questionDisplayRow}>
                        <QuestionDisplay content={answer.content} />
                      </div>
                ...
                }
                {
                  ...
                  SPEECH_ENABLED && (
                    <Button
                      ...
                      size="large"
                      icon={
                        appStateContext?.state.audioMuted ? (
                          <SpeakerMuteRegular />
                        ) : (
                          <Speaker224Regular />
                        )
                      ...
                      aria-label={
                        appStateContext?.state.audioMuted ? "Unmute" : "Mute"
                      }
                      tabIndex={0}
              ...
              <div className={styles.chatInputContainer}>
                <div className={styles.chatInput}>
                  <div className={styles.chatButtonsLeftContainer}>
                    ...
                    {file ? (
                      <Button
                        ...
                        className={styles.fileButtonSelected}
                        onClick={() => {
                          setFile(undefined);
                        }}
                        aria-label="Remove document button"
                      />
                    ) : (
                      <Button
                  ...
                  <QuestionInput
                    clearOnSend
                    placeholder={
                      appStateContext?.state.frontendSettings?.input_placeholder
                    }
                    disabled={isLoading}
                ...
                <div className={styles.footer}>
                  <Caption1 className={styles.footerText}>
                    <i>
                      AI may generate incorrect answers, please check citations
                      for accuracy.
                    </i>
                  </Caption1>
                  <ComplianceMessage />
      ...
      <Dialog open={!hideErrorDialog} onOpenChange={handleErrorDialogClose}>
        <DialogSurface>
          <DialogBody>
            <DialogTitle>{errorMsg?.title}</DialogTitle>
            <DialogContent>{errorMsg?.subtitle}</DialogContent>
            <DialogActions>
      ...
      <Dialog open={showFileDialog}>
        <DialogSurface>
          <DialogBody>
            ...
            <DialogActions>
              <DialogTrigger disableButtonEnhancement>
                <Button onClick={handleDocumentSubmit} appearance="secondary">
                  Attach
                </Button>
              </DialogTrigger>
              <DialogTrigger disableButtonEnhancement>
                <Button onClick={handleDocCancel} appearance="secondary">
                  Cancel
                </Button>
              </DialogTrigger>
            </DialogActions>
